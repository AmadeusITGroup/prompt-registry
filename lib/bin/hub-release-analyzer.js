...
// Extract and count unique bundles from all sources
// Fetches .collection.yml files to count bundles and primitives
function countBundlesFromAllSources(hubConfig, options = {}) {
  const { sourceFilter, verbose, spawnSync } = options;
  const allSources = getAllEnabledSources(hubConfig, { sourceFilter, verbose });
  const nonGitHubSources = [];
  const githubSources = [];

  for (const source of allSources) {
    if (source.type === 'awesome-copilot') {
      nonGitHubSources.push(source);
    } else if (source.type === 'github' || source.type === 'apm') {
      githubSources.push(source);
    }
  }

  const allBundleDetails = [];

  // Process all sources to find their collections and count primitives
  const allSourcesForScan = [
    ...nonGitHubSources.map((s) => ({ source: s, isGitHub: false })),
    ...githubSources.map((s) => ({ source: s, isGitHub: true })),
  ];

  for (const { source, isGitHub } of allSourcesForScan) {
  ...
  return {
    nonGitHubBundles,
    nonGitHubSourceCount: nonGitHubSources.length,
    githubSourceCount: githubSources.length,
    allBundleDetails,
    totalPrimitives,
  };
}
...
function extractBundleInfo(assetName) {
  ...
  if (versionMatch) {
    return {
      bundleId: normalizeBundleId(versionMatch[1].replace(/-$/, '')), // Remove trailing dash
...
function generateMarkdownReport(aggregated, outputDir, timestamp, args, totals = {}) {
  ...
  lines.push('');
  lines.push('| Bundle ID | Primitives | Downloads | Versions | Sources | Top Version | Top Downloads |');
  lines.push('|-----------|------------|-----------|----------|---------|-------------|---------------|');
  ...
  for (const b of sortedBundles) {
    ...
    lines.push(
      `| ${b.bundleId} | ${primitives} | ${formatNumber(b.totalDownloads)} | ${b.versionCount} | ${b.sourceCount} | ${b.topVersion.version || 'N/A'} | ${formatNumber(b.topVersion.downloads)} |`
    );
  }
  lines.push('');
...
async function main(opts = {}) {
  ...
  const allEnabledSources = getAllEnabledSources(hubConfig, {
  ...
  console.log(`Enabled sources: ${allEnabledSources.length}`);
  console.log(`Non-GitHub sources: ${bundleCounts.nonGitHubSourceCount} (${bundleCounts.nonGitHubBundles.length} bundles)`);
  ...
  if (args.format === 'md' || args.format === 'all') {
    const mdReport = generateMarkdownReport(aggregated, args.outputDir, timestamp, args, {
      totalSources: allEnabledSources.length,
