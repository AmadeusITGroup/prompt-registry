...
suite('E2E: Lockfile as Single Source of Truth Tests', () => {
    ...
    suite('12.2: Lockfile Portability - SourceId Format', () => {
         ...
         * 
         * The new sourceId format is `{sourceType}-{8-char-hash}` (e.g., `github-a1b2c3d4`)
        ...
        test('Requirement 2.1, 2.3: Lockfile with new sourceId format is portable across hub configurations', async function() {
            ...
            // Verify the new format is correct (type-8hexchars)
            assert.ok(
                /^[a-z]+-[a-f0-9]{8}$/.test(newFormatSourceId),
                `SourceId should match new format {type}-{8-hex-chars}, got: ${newFormatSourceId}`
            );
            assert.ok(
        ...
        test('Requirement 2.2, 2.3: Same source URL always produces same sourceId (deterministic)', async function() {
            ...
            // Verify format
            assert.ok(
                /^github-[a-f0-9]{8}$/.test(sourceId1),
                `SourceId should match format github-{8-hex-chars}, got: ${sourceId1}`
            );
        });
