 ...
 * - Property 3: SourceId format - output matches `{type}-{8-hex-chars}` pattern
...
suite('sourceIdUtils Property Tests', () => {
    ...
    suite('Property 3: SourceId Format', () => {
         ...
         * 
         * Property: generateHubSourceId output always matches the format `{type}-{8-hex-chars}`.
         * The hash portion must be exactly 8 lowercase hexadecimal characters.
         */
        test('generateHubSourceId output matches {type}-{8-hex-chars} pattern', async () => {
            await fc.assert(
                fc.property(
                    Generators.sourceType(),
                    Generators.url(),
                    (sourceType, url) => {
                        ...
                        // Build expected pattern: sourceType followed by hyphen and 8 hex chars
                        const expectedPattern = new RegExp(`^${sourceType}-[a-f0-9]{8}$`);
         ...
         * **Validates: Requirements 5.3**
         * 
         * Property: The sourceId always starts with the source type prefix.
         */
        test('generateHubSourceId output starts with source type', async () => {
            await fc.assert(
                fc.property(
                    Generators.sourceType(),
                    Generators.url(),
                    (sourceType, url) => {
                        ...
                        return result.startsWith(`${sourceType}-`);
                    }
                ),
                {
                    ...PropertyTestConfig.FAST_CHECK_OPTIONS,
         ...
         * 
         * Property: The hash portion is always exactly 8 characters.
         * This ensures collision resistance while keeping IDs readable.
         */
        test('generateHubSourceId hash portion is exactly 8 characters', async () => {
            await fc.assert(
                fc.property(
                    Generators.sourceType(),
                    Generators.url(),
                    (sourceType, url) => {
                        ...
                        // Extract hash portion (everything after the first hyphen)
                        const hashPortion = result.substring(sourceType.length + 1);

                        return hashPortion.length === 8;
                    }
                ),
                {
                    ...PropertyTestConfig.FAST_CHECK_OPTIONS,
         ...
         * **Validates: Requirements 5.3**
         * 
         * Property: The hash portion contains only lowercase hexadecimal characters.
         */
        test('generateHubSourceId hash portion contains only hex characters', async () => {
            await fc.assert(
                fc.property(
                    Generators.sourceType(),
                    Generators.url(),
                    (sourceType, url) => {
                        ...
                        return /^[a-f0-9]+$/.test(hashPortion);
                    }
                ),
                {
                    ...PropertyTestConfig.FAST_CHECK_OPTIONS,
    ...
    suite('Property 6: Hub Key Stability', () => {
         ...
         * **Validates: Requirements 4.6, 5.6**
         * 
         * Property: Hub key format is correct - either 8 hex chars or 8 hex chars + branch.
            ...
            await fc.assert(
                fc.property(
                    ...
                    (url, branch) => {
                        ...
                        // If branch is main, master, empty, or undefined, should be just 8 hex chars
                        if (!branch || branch === 'main' || branch === 'master' || branch === '') {
                            return /^[a-f0-9]{8}$/.test(result);
                        }

                        // Otherwise should be 8 hex chars followed by hyphen and branch
                        const expectedPattern = new RegExp(`^[a-f0-9]{8}-${escapeRegex(branch)}$`);
                        return expectedPattern.test(result);
                    }
                ),
                {
            ...
            await fc.assert(
                fc.property(
                    ...
                    (url, [branch1, branch2]) => {
                        ...
                        // Extract hash portion (first 8 characters)
                        const hash1 = result1.substring(0, 8);
                        const hash2 = result2.substring(0, 8);
