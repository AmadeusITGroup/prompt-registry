...
suite('RepositoryActivationService', () => {
    ...
    suite('checkAndPromptActivation()', () => {
        ...
        test('should not call enableRepositoryBundles automatically', async () => {
            // Arrange
            const lockfile = createMockLockfile(2);
            mockLockfileManager.read.resolves(lockfile);
            mockLockfileManager.getLockfilePath.returns('/repo/prompt-registry.lock.json');
            ...
            // Assert - should NOT call enableRepositoryBundles (files already in repo)
            assert.ok(!enableSpy.called, 'Should not call enableRepositoryBundles - files already in repository');
        });
