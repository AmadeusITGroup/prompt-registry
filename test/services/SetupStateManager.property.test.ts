...
suite('SetupStateManager - Property Tests', () => {
    ...
    // For any cancellation reason, markIncomplete() should set state to INCOMPLETE
    ...
    test('Property 7: Cancellation handling sets INCOMPLETE state (Req 8.1-8.2, 9.1-9.2)', async () => {
        const cancellationReasonArbitrary = fc.constantFrom(
            'hub_cancelled' as const,
            'auth_cancelled' as const
        );

        await fc.assert(
            fc.asyncProperty(
                cancellationReasonArbitrary,
                fc.integer({ min: 1, max: 5 }), // Number of cancellation calls
                async (reason, callCount) => {
                    globalStateData.clear();
                    SetupStateManager.resetInstance();

                    const manager = SetupStateManager.getInstance(mockContext, mockHubManager as any);
                    const testParams = formatTestParams({ reason, callCount });
                    ...
                    for (let i = 0; i < callCount; i++) {
                        await manager.markIncomplete(reason);
                    }
