...
export class VersionManager {
    ...
    static compareVersions(v1: string, v2: string): number {
        ...
        if (coerced1 && coerced2) {
            this.getLogger().debug(`Coerced versions for comparison: ${v1} -> ${coerced1.version}, ${v2} -> ${coerced2.version}`);
    ...
    static isUpdateAvailable(installedVersion: string, latestVersion: string): boolean {
        ...
        this.getLogger().debug(`Using compareVersions fallback for update check: ${installedVersion} vs ${latestVersion}`);
    ...
    static sortVersionsDescending(versions: string[]): string[] {
        ...
        for (const v of versions) {
            ...
            if (clean) {
                ...
                this.getLogger().debug(`Filtering out invalid version during sort: "${v}"`);
            }
        }
    ...
    static extractBundleIdentity(bundleId: string, sourceType: SourceType): string {
        ...
        if (match && match.index !== undefined) {
            ...
            this.getLogger().debug(`Extracted bundle identity: "${bundleId}" -> "${identity}"`);
            return identity;
        }
    ...
    static parseVersion(version: string): string | null {
        ...
        if (coerced) {
            this.getLogger().debug(`Coerced version: "${version}" -> "${coerced.version}"`);
